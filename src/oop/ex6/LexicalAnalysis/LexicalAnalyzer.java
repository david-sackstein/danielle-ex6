...
public class LexicalAnalyzer {
    ...
    public boolean isCommentOrEmptyLine(String line) {
        return RegexFactory.COMMENT_LINE.matcher(line).matches() ||
                RegexFactory.EMPTY_LINE.matcher(line).matches();
    }

    public boolean isReturnStatement(String line) {
        return RegexFactory.RETURN_STATEMENT.matcher(line).matches();
    }

    public boolean isEndOfBlock(String line) {
        return RegexFactory.END_OF_BLOCK.matcher(line).matches();
    }

    public boolean isNameOfVariable(String token) {
        return RegexFactory.NAME_OF_VARIABLE.matcher(token).matches();
    }
    ...
    public ArrayList<ArrayList<String>> splitDeclarationInt(String line) {
        return splitDeclaration(line, RegexFactory.DECLARATION_INT, RegexFactory.OPTIONAL_ASSIGNMENT_INT);
    }

    public ArrayList<ArrayList<String>> splitDeclarationString(String line) {
        return splitDeclaration(line, RegexFactory.DECLARATION_STRING, RegexFactory.OPTIONAL_ASSIGNMENT_STRING);
    }

    public ArrayList<ArrayList<String>> splitDeclarationBoolean(String line) {
        return splitDeclaration(line, RegexFactory.DECLARATION_BOOLEAN, RegexFactory.OPTIONAL_ASSIGNMENT_BOOLEAN);
    }

    public ArrayList<ArrayList<String>> splitDeclarationChar(String line) {
        return splitDeclaration(line, RegexFactory.DECLARATION_CHAR, RegexFactory.OPTIONAL_ASSIGNMENT_CHAR);
    }

    public ArrayList<ArrayList<String>> splitDeclarationDouble(String line) {
        return splitDeclaration(line, RegexFactory.DECLARATION_DOUBLE, RegexFactory.OPTIONAL_ASSIGNMENT_DOUBLE);
    }

    public ArrayList<ArrayList<String>> splitAssignmentInt(String line) {
        return splitAssignmentLine(line, RegexFactory.ASSIGNMENT_INT_LINE, RegexFactory.ASSIGNMENT_INT);
    }

    public ArrayList<ArrayList<String>> splitAssignmentString(String line) {
        return splitAssignmentLine(line, RegexFactory.ASSIGNMENT_STRING_LINE, RegexFactory.ASSIGNMENT_STRING);
    }

    public ArrayList<ArrayList<String>> splitAssignmentBoolean(String line) {
        return splitAssignmentLine(line, RegexFactory.ASSIGNMENT_BOOLEAN_LINE, RegexFactory.ASSIGNMENT_BOOLEAN);
    }

    public ArrayList<ArrayList<String>> splitAssignmentChar(String line) {
        return splitAssignmentLine(line, RegexFactory.ASSIGNMENT_CHAR_LINE, RegexFactory.ASSIGNMENT_CHAR);
    }

    public ArrayList<ArrayList<String>> splitAssignmentDouble(String line) {
        return splitAssignmentLine(line, RegexFactory.ASSIGNMENT_DOUBLE_LINE, RegexFactory.ASSIGNMENT_DOUBLE);
    }

    public ArrayList<ArrayList<String>> splitMethodDeclaration(String line) {
        return splitMethod(line, RegexFactory.METHOD_DECLARATION, RegexFactory.ARGUMENT_DECLARATION);
    }

    public ArrayList<ArrayList<String>> splitMethodInvocation(String line) {
        return splitMethod(line, RegexFactory.METHOD_INVOCATION, RegexFactory.ARGUMENT_VALUE);
    }

    public ArrayList<ArrayList<String>> splitBlockCondition(String line) {
        return splitDeclaration(line, RegexFactory.CONDITION_BLOCK, RegexFactory.CONDITION_EXPRESSION);
    }
    ...
    private boolean isInt(String token) {
        return RegexFactory.INT_LITERAL.matcher(token).matches();
    }

    private boolean isBoolean(String token) {
        return RegexFactory.BOOLEAN_LITERAL.matcher(token).matches();
    }

    private boolean isDouble(String token) {
        return RegexFactory.DOUBLE_LITERAL.matcher(token).matches();
    }

    private boolean isString(String token) {
        return RegexFactory.STRING_LITERAL.matcher(token).matches();
    }

    private boolean isChar(String token) {
        return RegexFactory.CHAR_LITERAL.matcher(token).matches();
    }
}
